//
//  CodingLayoutViewController.swift
//  ForGitExperience
//
//  Created by Lotte on 12/08/2019.
...
class CodingLayoutViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "설정" // 1. 내비게이션 바 타이틀을 입력한다.

        // 2. 이메일 레이블을 생성하고 영역과 기본 문구를 설정한다.
        let lblEmail = UILabel()
        lblEmail.frame = CGRect(x: 30, y: 100, width: 100, height: 30)
        lblEmail.text = "이메일"
        lblEmail.font = UIFont.systemFont(ofSize: 14) // 3. 레이블의 폰트를 설정한다.
        ...
        self.paramUpdate.setOn(true, animated: true) // 스위치가 On 되어 있는 상태를 기본값으로 설정한다.
